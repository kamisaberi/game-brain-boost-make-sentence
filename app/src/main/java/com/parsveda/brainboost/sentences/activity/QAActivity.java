...
import com.parsveda.brainboost.sentences.model.Answers;
...
//        btnBack.getHitRect(rect);
    ...
    public void randomButtons() {
        ...
        Questions questions = Globals.questions.getQuestionsWithAnswersThatHaveWordCount(count);
        Globals.selectedQuestion = questions.get(rand.nextInt(questions.size()));

        //Preset preset = presetsForRand.get(Globals.selectedPresetId);

        txtQuestion.setText(Globals.selectedQuestion.getValue());

        Globals.textToSpeech.speak(txtQuestion.getText().toString(), TextToSpeech.QUEUE_FLUSH, null);

        Answers answers = Globals.selectedQuestion.getAnswersThatHasWordCount(count);

        Answer answer = answers.get(rand.nextInt(answers.size()));

        Globals.selectedQuestion.setSelectedAnswer(answer);

        String[] words = answer.getWords();
        ...
        while (true) {
            ...
            dpsize = Math.max(len * 20, 50);
            params.width = Utils.convertDpToPixel(getResources(), dpsize);
            params.height = Utils.convertDpToPixel(getResources(), 50);
...
//                        timer.start();
            ...
            if (Globals.currentStage.getTouchOrderType() == StageTouchOrderType.NORMAL) {
                ...
                button.setTextColor(ContextCompat.getColor(QAActivity.this, R.color.mid_blue));
            } else if (Globals.currentStage.getTouchOrderType() == StageTouchOrderType.REVERSE) {
                //button.setTextColor(getResources().getColor(R.color.dark_teal));
                button.setTextColor(ContextCompat.getColor(QAActivity.this, R.color.dark_teal));
            } else if (Globals.currentStage.getTouchOrderType() == StageTouchOrderType.COMPLEX) {
                if (Globals.currentStage.partTouchOrderType == StageTouchOrderType.NORMAL) {
                    button.setTextColor(ContextCompat.getColor(QAActivity.this, R.color.mid_blue));
                } else if (Globals.currentStage.partTouchOrderType == StageTouchOrderType.REVERSE) {
